...
## 3.3 Tileset / Palette 面板

分页：
- Palette 顶部有 `上一页 / 下一页 / 页码`，只渲染当前页以保证性能。

缩略图缩放：
- 页码右侧有 `小 / 中 / 大` 三档缩放按钮，用于调整 palette 里 tile 按钮的尺寸。

搜索过滤：
- 点击搜索框进入输入状态（会显示 `|` 光标）。
- 支持输入：字母、数字、空格、`-`、`,`。
- `Backspace` 删除；`Enter`/`Esc` 退出输入。
- 点击 `清空` 可清除过滤并回到第 1 页。

过滤语法：
- `12`：只显示 tile index = 12
- `10-20`：显示 tile index 在 10..=20 的范围（`20-10` 也可）
- 关键字：对 `index` 或 `x,y`（tile 坐标）做子串匹配，例如 `3,1` 或 `3x1`
...
顶部菜单栏（新增）：
- `File`：打开 tileset、新建/保存/读取/导入/导出地图
- `Edit`：撤销/重做
- `View`：网格开关
- `Map`：地图尺寸预设、Shift 模式切换
