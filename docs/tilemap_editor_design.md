...
## 2.1 与 RPG Maker 数据库图块的差异（对齐说明）

当前编辑器的 tileset 只保存“图块图像与索引”，不包含 RM 数据库图块的元数据：
- 自动图块规则（A1~A5 的拼接/地形规则）
- 通行/优先级/地形标记等属性
- A/B/C/D/E 分区与标签页

因此：现阶段更像“纯图层绘制器”，而非 RM 的“数据库图块编辑器”。

---

## 2.2 计划：补齐数据库式元数据（草案）

目标：在不影响现有绘制/渲染流程的前提下，新增“数据库式 tileset 元数据与叠加层”。

**阶段 A：数据模型（不动 UI）**
1) 扩展 tileset 元数据结构：
	- `tileset_meta.ron`：每个 tileset 的规则与属性表
	- 包含：通行/优先级/地形标签/自动图块定义（占位）
2) `tilemap_format`：支持保存/读取 tileset_meta（与 map.ron 解耦）

**阶段 B：渲染与编辑入口**
3) 增加“元数据图层（overlay）”渲染：
	- 通行/RegionId 可视化网格（仅预览）
4) 增加编辑模式切换（Tile / Meta）：
	- Meta 模式下使用独立笔刷写入 RegionId/Passage

**阶段 C：自动图块（可选）**
5) 定义 autotile 规则格式（RM 子集）
6) 运行时自动拼接与编辑器预览

说明：以上计划不会影响现有 map.ron 绘制数据；先补“元数据层 + 规则存储”，再做自动图块。
